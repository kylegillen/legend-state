...
const { initializePathType } = state.internal;
let validateMap;
...
function transformObjectWithPath(obj, path, pathTypes, fieldTransforms) {
    const constructed = state.constructObjectWithPath(path, pathTypes, obj);
    ...
    return { path: transformedPath, obj: state.deconstructObjectWithPath(transformedPath, pathTypes, transformed) };
}
